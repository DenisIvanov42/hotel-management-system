...
export default function App() {
  ...
  // 1. We create a "Ref" to directly grab the HTML element of the sticky header
  ...
  // 2. We use a Native Event Handler to forcefully stop the scroll
  const handleWheel = useCallback((e: WheelEvent) => {
    e.preventDefault(); // Because passive is false below, this now actually stops the table from scrolling!
  ...
  // 3. We attach the Native Event Listener when the component loads
  useEffect(() => {
    const thead = headerRef.current;
    if (thead) {
      // The magic key: passive: false
      thead.addEventListener('wheel', handleWheel, { passive: false });
    }
    return () => {
      if (thead) {
        thead.removeEventListener('wheel', handleWheel);
      }
    };
  }, [handleWheel]);
    ...
    <div className="h-screen flex flex-col bg-slate-50 p-6 font-sans text-slate-800 overflow-hidden">
      ...
      <div className="mb-4 flex items-baseline justify-between shrink-0">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* FIXED: whitespace-nowrap and min-w-[300px] stops the text from wrapping! */}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex-1 min-h-0 overflow-auto relative">
        <table className="w-full text-sm text-left border-collapse">
          ...
          <thead 
            ref={headerRef} // We attach our reference here instead of using React's onWheel
            ...
            <tr>
              ...
              {weekDays.map((date, index) => {
                ...
                return (
                  <th 
                    key={index} 
                    className={`p-2 border-r border-b border-slate-200 text-center min-w-[120px] sticky top-0 z-40 bg-slate-100 transition-colors cursor-ew-resize shadow-[0_1px_0_#e2e8f0] ${
              ...
              <tr key={room.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors group">
                ...
                {weekDays.map((date, index) => {
                  ...
                  return (
                    <td 
                      key={index} 
                      className={`p-0 border-r border-slate-100 relative cursor-pointer hover:bg-indigo-50 transition-colors h-14 z-0 ${
