    ...
    <div className="h-screen flex flex-col bg-slate-50 p-6 font-sans text-slate-800 overflow-hidden relative">
      <div className="mb-4 flex items-baseline justify-between shrink-0">
        ...
        <div className="flex items-center gap-4">
          ...
          <div 
            ref={pillRef} 
            className="inline-flex rounded-lg border border-slate-300 bg-white p-1 shadow-sm cursor-ns-resize"
            title="Скролирайте тук с мишката, за да сменяте седмиците"
          >
            <button 
              onClick={() => setCurrentWeekStart(prev => subWeeks(prev, 1))} 
              className="p-1.5 text-slate-600 hover:bg-slate-100 hover:text-indigo-600 rounded-md transition-colors"
            >
            ...
            <tr>
              ...
              {weekDays.map((date, index) => {
                ...
                return (
                  <th 
                    key={index} 
                    className={`p-2 border-r border-b border-slate-200 text-center min-w-[140px] sticky top-0 z-40 shadow-[0_1px_0_#e2e8f0] transition-colors ${isCurrentDay ? 'text-indigo-700 shadow-[inset_0_-3px_0_#4f46e5]' : ''} ${isHovered ? 'bg-indigo-100' : 'bg-slate-100'}`}
                  >
          ...
          <tfoot className="sticky bottom-0 z-30 bg-slate-100 shadow-[0_-1px_0_#cbd5e1]">
            <tr>
              ...
              {weekDays.map((date, index) => {
                ...
                let totalGuests = 0;
                // We still calculate revenue here, but we just don't render it in the footer cell anymore!
                bookings.forEach(b => {
                ...
                return (
                  <td 
                    ...
                    onMouseEnter={() => setHoveredDateStr(dateStr)}
                    onMouseLeave={() => setHoveredDateStr(null)}
                    className={`p-2 border-r border-slate-200 text-center cursor-pointer transition-all ${isHovered ? 'bg-indigo-100 text-indigo-900 shadow-inner' : 'text-slate-600'}`}
                  >
                    <div className="text-sm font-bold">{totalGuests} гости</div>
                    {/* The revenue div is gone! Just the note indicator below */}
                    {hasNote ? (
                      <div className="mx-auto mt-1.5 w-2 h-2 bg-amber-500 rounded-full shadow-sm"></div>
                    ) : (
                      <div className="h-3.5"></div> /* Empty spacer so layout doesn't jump */
                    )}
                  </td>
                );
              })}
      ...
      {summaryDate && (() => {
        // Calculate the pop-up stats here on the fly
        let modalGuests = 0;
        let modalRevenue = 0;
        bookings.forEach(b => {
          ...
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[80] flex items-center justify-center transition-opacity" onClick={(e) => { if (e.target === e.currentTarget) setSummaryDate(null); }}>
            <div className="bg-white rounded-2xl shadow-2xl w-[400px] overflow-hidden transform scale-100">
              ...
              <div className="p-6">
                ...
                {/* NEW: Stats bar displaying the hidden revenue and guest count */}
                <div className="flex justify-center gap-3 mb-6">
                  <div className="bg-slate-100 text-slate-700 px-3 py-1.5 rounded-lg text-sm font-bold shadow-sm border border-slate-200">
                    👥 {modalGuests} гости
                  </div>
                  <div className="bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-lg text-sm font-bold shadow-sm border border-emerald-200">
                    💶 Общо: {modalRevenue.toFixed(0)} €
                  </div>
                </div>

                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">Дневни бележки и задачи</label>
                {/* Notice: No placeholder attribute here anymore! */}
